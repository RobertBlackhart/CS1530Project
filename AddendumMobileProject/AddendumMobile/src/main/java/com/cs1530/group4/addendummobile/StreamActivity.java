...
public class StreamActivity extends ActionBarActivity implements ActionBar.OnNavigationListener
{
	...
	private View makeHeader()
	{
		...
		try
		{
			String[] names = getResources().getAssets().list("universities");
			Random random = new Random();
			int index = random.nextInt(names.length);
			background.setImageBitmap(BitmapFactory.decodeStream(getResources().getAssets().open("universities/" + names[index])));
		}
		catch(IOException e)
		{
			e.printStackTrace();
	...
	private void fillNavigationMenu()
	{
		...
		actionBar.setNavigationMode(ActionBar.NAVIGATION_MODE_LIST);
		actionBar.setListNavigationCallbacks(new ArrayAdapter<String>(context, android.R.layout.simple_dropdown_item_1line, courseList), this);
	}
